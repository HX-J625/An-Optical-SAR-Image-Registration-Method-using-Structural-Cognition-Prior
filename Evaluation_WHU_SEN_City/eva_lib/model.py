...
from exp_lib.modules import *
from .MDSE import MDSE
...
class CBAMWT(nn.Module):
    def __init__(self, in_planes, target_channels=128, ratio=16, kernel_size=7):
        super(CBAMWT, self).__init__()
        self.channel_attn = ChannelAttention(in_planes, ratio)      
        self.spatial_attn = SpatialAttention(kernel_size)           
        self.mapper = nn.Conv2d(in_planes, target_channels, 1, bias=False)
    ...
    def forward(self, x):
        ...
        sp_weight = self.spatial_attn(x_ch)                 
        weight = sp_weight * x_ch                           
...
class MMNet(nn.Module):
    def __init__(self):
        ...
        self.enc = Encoder()
        self.det = Superhead()
        self.cbamwt1 = CBAMWT(in_planes=12, target_channels=3)
        self.cbamwt2 = CBAMWT(in_planes=12, target_channels=3)

    def forward1(self, imgs):
        MDSE_feat = self.MDSE(imgs[:, 0:1, :, :])           
        weight = self.cbamwt1(MDSE_feat)                    
        modulated_img = imgs * weight                
        adapter_out = self.ada1(modulated_img)
        feat = self.enc(adapter_out)
        score = self.det(feat.pow(2))
        return F.normalize(feat, dim=1), score

    def forward2(self, imgs):
        MDSE_feat = self.MDSE(imgs[:, 0:1, :, :])
        weight = self.cbamwt2(MDSE_feat)
        modulated_img = imgs * weight
        adapter_out = self.ada2(modulated_img)
        feat = self.enc(adapter_out)
        score = self.det(feat.pow(2))
        return F.normalize(feat, dim=1), score
