...
parser.add_argument("--feature_name", type=str, default='OSdataset',help='Name of feature')
...
for subset in subsets:
    ...
    for num in [1024,2048,4096]:
        ...
        SR = len(image_list) - int(mask.sum()) - num_black_list   
        total = len(image_list)                                  
        SRR = SR / total if total > 0 else 0.0
        print('SR: {}  | total: {}  | SRR: {:.4f}'.format(SR, total, SRR))

        success_cnt = int((~mask).sum())
        if success_cnt == 0:
            print('fail to reproject')
            mean_ok = float('nan')
        else:
            mean_ok = float(errs[~mask].mean())
        ...
        log_file.write('SRR: {:.4f} (SR={}/{})\n'.format(SRR, SR, len(image_list)))
